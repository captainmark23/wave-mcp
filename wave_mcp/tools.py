...
def _write_private(path: Path, content: str) -> None:
    """Write text to a file with owner-only permissions (0o600)."""
...
async def wave_list_all_sessions(params: ListAllSessionsInput, ctx: Context) -> str:
    ...
    while True:
        ...
        await _wait_for_rate_limit(ctx)
    ...
    if truncated:
        lines.append(f"\n---\n*Results truncated at {max_pages} pages. More sessions may exist.*")
    return "\n".join(lines)
...
async def wave_download_audio(params: DownloadAudioInput, ctx: Context) -> str:
    ...
    _validate_download_url(audio_url)
...
async def wave_export_archive(params: ExportArchiveInput, ctx: Context) -> str:
    ...
    for batch_start in range(0, total, MAX_BULK_SESSIONS):
        ...
        try:
            ...
            sanitized_err = _redact_tokens(str(exc))[:200]
            errors.append(
        ...
        for s in export_data.get("sessions", []):
            ...
            try:
                ...
                logger.debug("Malformed timestamp for session %s: %s", sid, timestamp)
            ...
            try:
                ...
                # Save summary (sanitize to prevent markdown injection, owner-only perms)
                if s.get("summary"):
                    _write_private(
                        folder_path / "summary.md",
                        f"# {_sanitize_md(title)}\n\n**Date:** {timestamp}  \n**Duration:** {_format_duration(s.get('duration_seconds'))}  \n\n## Summary\n\n{_sanitize_md(s['summary'])}\n",
                    )
                    metadata["has_summary"] = True

                # Save transcript (sanitize to prevent markdown injection, owner-only perms)
                if s.get("transcript"):
                    _write_private(
                        folder_path / "transcript.md",
                        f"# Transcript: {_sanitize_md(title)}\n\n**Date:** {timestamp}  \n**Duration:** {_format_duration(s.get('duration_seconds'))}  \n\n---\n\n{_sanitize_md(s['transcript'])}\n",
                    )
                ...
                logger.warning("Failed to write session %s to disk: %s", sid, exc)
                errors.append({"id": sid, "error": f"File write failed: {exc}"})
    ...
    index = {
        ...
        **({"pagination_truncated": True} if pagination_truncated else {}),
    }
    ...
    if pagination_truncated:
        result += f"\n**Warning:** Pagination was truncated at {max_pages} pages. More sessions may exist.\n"
    if errors:
        result += "\n**Error details:**\n"
